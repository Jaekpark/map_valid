...
#define	RETURN	{ if (colors) free(colors); if (tab) free(tab); \
		  if (colors_direct) free(colors_direct); \
                  if (img) mlx_destroy_image_beta(xvar, img);   \
                  return ((void *)0); }
...
void	*mlx_int_parse_xpm(void *xvar,void *info,int info_size,char *(*f)(), int *width, int *height)
{
  ...
  if (!(img = mlx_new_image_beta(xvar,*width,*height)))
    RETURN;
  data = mlx_get_data_addr_beta(img, &opp, &sl, &endian);
  opp = 4;
...
void	*mlx_xpm_file_to_image_beta(void *xvar,char *file,int *width,int *height)
{
  int	fd;
  int	size;
  char	*ptr;
...
void	*mlx_xpm_to_image_beta(void *xvar,char **xpm_data,int *width,int *height)
{
