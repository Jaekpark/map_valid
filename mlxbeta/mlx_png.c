...
int	mipng_fill_img(void *img, unsigned char *buf, png_info_t *pi)
{
  ...
  ibuf = (unsigned char *)mlx_get_data_addr_beta(img, &bpp, &iline, &endian);
...
void	*mlx_int_parse_png(void *xvar, unsigned char *fptr, int size, int *width, int *height)
{
  ...
  if (!(img = mlx_new_image_beta(xvar, pi.width, pi.height)))
    {
  ...
  *height = pi.height;
  if ((err = mipng_data(img, dat, &pi)))
    {
      mlx_destroy_image_beta(xvar, img);
      warnx("mlx PNG error : %s", mipng_err[err]);
      return ((void *)0);
    }
  return (img);
...
void	*mlx_png_file_to_image_beta(void *xvar, char *file, int *width, int *height)
{
  int			fd;
  int			size;
  unsigned char		*ptr;
